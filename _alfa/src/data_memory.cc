...
data_memory_t::data_memory_t(const char *m_mem_state, uint64_t *m_ticks,
                             uint64_t m_memory_size,
                             uint64_t m_code_segment_size, uint64_t m_latency)
    : cache(0), ticks(m_ticks), memory(0), accessed(0),
      memory_size(m_memory_size), code_segment_size(m_code_segment_size),
      num_dwords(m_memory_size >> 3), latency(m_latency), resp_ticks(0),
      req_block(0) {
...
void data_memory_t::load_block(uint64_t m_addr, uint64_t m_block_size) {
  ...
  if ((m_addr + m_block_size) > memory_size) {
    ...
    cerr << "Error: memory address " << m_addr << " is in the code segment"
         << endl;
    exit(1);
  }

  // Mark all doublewords in the requested block are accessed.
  for (uint64_t i = 0; i < m_block_size >> 3; i++) {
    accessed[(m_addr >> 3) + i] = true;
  }
...
// Load initial memory state.
void data_memory_t::load_mem_state(const char *m_mem_state) {
  // Open a memory state file.
  std::istringstream file_stream(m_mem_state);
  ...
  while (getline(file_stream, line)) {
    ...
    if (!is_num_str(addr_str) || !is_num_str(data_str) || !addr_str.length() ||
        !data_str.length()) {
      cerr << "Error: invalid memory address and/or data " << addr_str << " = "
           << data_str << " at line #" << line_num << " of mem_state" << endl;
      exit(1);
    }
    ...
    // Check if multiple different values are defined at the same memory
    // address.
