...
class data_memory_t {
public:
  data_memory_t(const char *m_mem_state, uint64_t *m_ticks,
                uint64_t m_memory_size, uint64_t m_code_segment_size,
                uint64_t m_latency = 0);
  ~data_memory_t();

  void connect(data_cache_t *m_cache); // Connect to the upper-level cache.
  void load_block(uint64_t m_addr,
                  uint64_t m_block_size); // Load a memory block.
  void run();                             // Run the data memory.
  void print_state() const;               // Print memory state.

private:
  void load_mem_state(const char *m_mem_state); // Load initial memory state.
