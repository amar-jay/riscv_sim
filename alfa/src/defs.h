...
static kite_opcode_type kite_op_type[num_kite_opcodes]
...
#define get_opcode(m_string)                                                   \
  (kite_opcode)                                                                \
      distance(&kite_opcode_str[0],                                            \
               find(&kite_opcode_str[0], &kite_opcode_str[num_kite_opcodes],   \
                    m_string.c_str()))

// Get the kite_opcode_type of m_op
#define get_op_type(m_op) kite_op_type[m_op]

// Get the execution latency of m_op
#define get_op_latency(m_op) kite_op_latency[m_op]

// Convert a string to kite_reg
#define get_regnum(m_string)                                                   \
  (kite_reg) distance(                                                         \
      &kite_reg_str[0],                                                        \
      find(&kite_reg_str[0], &kite_reg_str[num_kite_regs], m_string.c_str()))

// Convert a string to 64-bit integer.
#define get_imm(m_string) (int64_t)strtoll(m_string.c_str(), 0, 10)

// Check if a string is a positive integer.
#define is_pos_num_str(m_string)                                               \
  (m_string.size() && (m_string.find_first_not_of(numbers) == string::npos))

// Check if a string is a negative integer.
#define is_neg_num_str(m_string)                                               \
  ((m_string[0] == '-') && is_pos_num_str(m_string.substr(1, string::npos)))
...
#define is_reg_str(m_string) (get_regnum(m_string) != num_kite_regs)
