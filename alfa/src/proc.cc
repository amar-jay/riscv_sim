...
proc_t::proc_t()
    : stalls(0), num_insts(0),
#ifdef BR_PRED
      num_br_predicts(0), num_br_mispredicts(0), num_br_tgt_mispredicts(0),
#endif
...
void proc_t::init(const char *m_program_code, const char *m_mem_state,
                  const char *m_reg_state,
  ...
  reg_file = new reg_file_t(m_reg_state); // Create a register file.
  alu = new alu_t(&ticks);                // Create an ALU.

  data_memory = new data_memory_t(m_mem_state, &ticks, 4096,
                                  inst_memory->num_insts() << 2,
                                  0);                // Create a data memory.
...
#else // No branch prediction is used. The next PC of a branch is set here to
  ...
  if (mem_wb_preg.is_free()) {
    ...
    if (data_cache->is_free() && (mem_inst = ex_mem_preg.read())) {
      ...
      if (mem_inst->op == op_ld) {
        data_cache->read(mem_inst);
      } else if (mem_inst->op == op_sd) {
        data_cache->write(mem_inst);
      }
    }
...
void proc_t::print_stats() {
  ...
  cout << "Number of branch target mispredictions = " << num_br_tgt_mispredicts
       << endl;
  cout << "Branch prediction accuracy = " << fixed
       << (num_br_predicts ? double(num_br_predicts - num_br_mispredicts -
                                    num_br_tgt_mispredicts) /
