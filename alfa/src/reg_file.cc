  ...
  if (m_inst->rs1_num > 0) {
    ...
    if ((prod_inst = dep[m_inst->rs1_num])) {
      ...
      if (prod_inst->rd_ready) {
        m_inst->rs1_val = prod_inst->rd_val;
      }
    ...
    // Rs1 is clear to go.
      ...
      m_inst->rs1_val = regs[m_inst->rs1_num];
    }
  }
  if (m_inst->rs2_num > 0) {
    ...
    if ((prod_inst = dep[m_inst->rs2_num])) {
      ...
      if (prod_inst->rd_ready) {
        m_inst->rs2_val = prod_inst->rd_val;
      }
    ...
    // Rs2 is clear to go.
      ...
      m_inst->rs2_val = regs[m_inst->rs2_num];
    }
  }
...
void reg_file_t::flush() { memset(dep, 0, sizeof(dep)); }

// Load initial register file state.
void reg_file_t::load_reg_state(const char *m_reg_state) {
  std::istringstream file_stream(m_reg_state);
  ...
  while (getline(file_stream, line)) {
    ...
    if (!is_reg_str(reg_name) || (reg_num >= num_kite_regs)) {
      cerr << "Error: invalid register name " << reg_name << " at line #"
           << line_num << " of reg_state" << endl;
      exit(1);
    }
    if (line.length() <= 0) {
      cerr << "Error: invalid register value for " << reg_name << " at line #"
           << line_num << " of reg_state" << endl;
      exit(1);
    }
