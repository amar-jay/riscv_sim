...
proc_t::proc_t(int8_t *is_debug_on, int8_t *is_data_fwd_on,
               int8_t *is_br_pred_on)
    : stalls(0), num_insts(0), is_debug_on(is_debug_on),
      is_data_fwd_on(is_data_fwd_on), is_br_pred_on(is_br_pred_on),
      num_flushes(0), ticks(0),
      pc(4), // The first instruction is loaded from PC = 4.
      inst_memory(0), br_predictor(0), br_target_buffer(0), reg_file(0), alu(0),
      data_memory(0), data_cache(0) {
...
void proc_t::init(const char *m_program_code, const char *memory_state,
                  const char *reg_state) {
  inst_memory =
      new inst_memory_t(m_program_code); // Create an instruction memory.
  br_predictor = new br_predictor_t(16); // Create a branch predictor.
  br_target_buffer =
      new br_target_buffer_t(16);       // Create a branch target buffer.
  reg_file = new reg_file_t(reg_state); // Create a register file.
  alu = new alu_t(&ticks, is_debug_on, is_data_fwd_on);
  ; // Create an ALU.

  data_memory =
      new data_memory_t(memory_state, &ticks, 4096, 0); // Create a data memory.
  data_cache = new data_cache_t(&ticks, 1024, is_debug_on, is_data_fwd_on, 8,
...
void proc_t::run(std::ostringstream &program_log) {
  ...
  while (!ticks || if_id_preg.read() || id_ex_preg.read() ||
         ex_mem_preg.read() || mem_wb_preg.read() || !alu->is_free() ||
         !data_cache->is_free()) {
...
void proc_t::writeback(std::ostringstream &program_log) {
  ...
  if (inst) {
    ...
    if (*is_debug_on) {
      program_log << ticks << " : writeback : " << get_inst_str(inst, true)
                  << endl;
    }
    // Update the branch predictor and branch target buffer for conditional
    // branches.
    if (inst->branch_target) {
      if (*is_br_pred_on) {
        ...
        if (inst->pred_target != inst->branch_target) {
          ...
          if (*is_debug_on) {
            program_log << ticks << " : pipeline flush : restart at PC = " << pc
                        << endl;
          }
        }
      } else {
        // No branch prediction is used. The next PC of a branch is set here to
        // avoid speculative executions.
        pc = inst->branch_target;
      }
    }
...
void proc_t::memory(std::ostringstream &program_log) {
  ...
  if (mem_wb_preg.is_free()) {
    ...
    if (data_cache->is_free() && (mem_inst = ex_mem_preg.read())) {
      ...
      if (mem_inst->op == op_ld) {
        data_cache->read(mem_inst, program_log);
      } else if (mem_inst->op == op_sd) {
        data_cache->write(mem_inst, program_log);
      }
    }
  ...
  if (*is_debug_on) {
    inst_t *inst = mem_wb_preg.read();
    if (inst) {
      program_log << ticks << " : memory : " << get_inst_str(inst, true)
                  << endl;
    }
  }
}
...
void proc_t::execute(std::ostringstream &program_log) {
  inst_t *inst = 0;
  // Execution stage makes a progress only if the EX/MEM pipeline register is
  // free.
  if (ex_mem_preg.is_free()) {
    // An instruction is read from the ID/EX pipeline register.
    if ((inst = id_ex_preg.read()) && alu->is_free()) {
      // Remove the instruction from the ID/EX pipeline register, and let the
      // ALU execute the instruction.
      id_ex_preg.clear();
  ...
  if (*is_debug_on) {
    if ((inst = ex_mem_preg.read())) {
      program_log << ticks << " : execute : " << get_inst_str(inst, true)
                  << endl;
    }
  }
}
...
void proc_t::decode(std::ostringstream &program_log) {
  ...
  if (id_ex_preg.is_free()) {
    ...
    if ((inst = if_id_preg.read())) {
      ...
      if (!reg_file->dep_check(inst)) {
        ...
        if (inst->op == op_jalr) {
          pc = (inst->rs1_val + inst->imm) & -2;
        } else if (inst->op == op_jal) {
          pc = inst->pc + (inst->imm << 1);
        }
      }
    }
  }
  if (*is_debug_on) {
    if ((inst = id_ex_preg.read())) {
      program_log << ticks << " : decode : " << get_inst_str(inst, true)
                  << endl;
    }
  }
}
  ...
  if (if_id_preg.is_free()) {
    ...
    if ((inst = inst_memory->read(pc))) {
      ...
      if (get_op_type(inst->op) == op_sb_type) {
        if (*is_br_pred_on) {
          // Set the PC to a branch target if the branch is predicted to be
          // taken.
          inst->pred_taken = br_predictor->is_taken(inst->pc);
          pc = inst->pred_target =
              inst->pred_taken ? br_target_buffer->get_target(inst->pc) : pc;
        } else {
...
void proc_t::print_stats(std::ostringstream &program_log) {
  ...
  if (*is_br_pred_on) {
    ...
    program_log << "Branch prediction accuracy = " << fixed
                << (num_br_predicts
                        ? double(num_br_predicts - num_br_mispredicts) /
                              double(num_br_predicts)
                        : 0)
                << " (" << num_br_predicts - num_br_mispredicts << "/"
